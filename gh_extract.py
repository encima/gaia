...
def run_query(query, variables):
    ...
    if request.status_code == 200:
        ...
        raise Exception("Query failed to run with a return code of {}.".format(request.status_code))
...
def fetch_all_comments(owner, repo, issue_number):
    ...
    while has_next_page:
        ...
        comments = result.get('data', {}).get('repository', {}).get('discussion', {}).get('comments', {}).get('edges', [])
        
        for comment in comments:
            comment_data = comment.get('node', {})
            all_comments.append(comment_data)

        page_info = result.get('data', {}).get('repository', {}).get('discussion', {}).get('comments', {}).get('pageInfo', {})
...
def fetch_issues_and_write_to_json(repo):
    ...
    with open(f'repos/{repo_name}_discussions.json', 'w', encoding='utf-8') as file:
        ...
        while has_next_page:
            ...
            '''
            variables = {'owner': owner, 'repo': repo_name, 'issuesCursor': issues_cursor}
            result = run_query(issues_query, variables)
            issues = result.get('data', {}).get('repository', {}).get('discussions', {}).get('edges', [])
            ...
            for issue in issues:
                ...
                comments = fetch_all_comments(owner, repo_name, issue_data.get('number', 0))
                issue_data['comments'] = comments
                issue_json = json.dumps(issue_data, indent=4)
                file.write(issue_json)

            page_info = result.get('data', {}).get('repository', {}).get('discussions', {}).get('pageInfo', {})
